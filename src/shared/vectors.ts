...
export function generateProductVector(
  ...
  const brandOffset = Array.from({ length: VECTOR_DIM }, () => (brandRng() * 2 - 1) * 0.15)

  const prodRng = seededRandom(productSeed)
  const noise = Array.from({ length: VECTOR_DIM }, () => (prodRng() * 2 - 1) * 0.08)

  const vec = centroid.map((c, i) => c * 0.8 + brandOffset[i] + noise[i])
  return normalize(vec)
}
